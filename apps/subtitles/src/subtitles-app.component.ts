...
export class SubtitlesAppComponent implements AfterViewInit,OnDestroy {
  ...
  importSrt(input:HTMLInputElement){const file=input.files?.[0];if(!file)return;file.text().then(text=>{this.cues=this.parseSrt(text);this.current=this.cues.length?0:-1;this.dirty=!!this.cues.length;this.refreshNativeTrack();this.centerActiveCue(this.current);this.message=`${this.cues.length} ${this.t('subtitles')}`;});input.value='';}
