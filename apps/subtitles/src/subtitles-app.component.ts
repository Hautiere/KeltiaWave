...
const LABELS: Record<string, Record<UiLanguage,string>> = {
  ...
  apply:{fr:'Appliquer',en:'Apply',br:'Arloañ',cy:'Gosod'},
...
export class SubtitlesAppComponent implements AfterViewInit,OnDestroy {
  ...
  maxChars=42; maxLines=2; maxDuration=6; private request?:Subscription; private progressTimer?:number; private nativeTrack?:TextTrack; private importedSrtName='';
  ...
  generate(){if(!this.file||this.busy)return;this.busy=true;this.error='';this.upload=0;this.work=0;this.message=this.t('processing');
    this.request=this.api.transcribe(this.file,this.language,this.engine).subscribe({next:event=>{this.upload=event.upload;if(event.upload===100&&!event.result)this.startProgress();if(event.result){this.stopProgress();this.work=100;this.cues=this.format(event.result.segments?.length?event.result.segments:[{start:0,end:this.duration||4,text:event.result.text||''}]);this.current=this.cues.length?0:-1;this.dirty=!!this.cues.length;this.refreshNativeTrack();this.centerActiveCue(this.current);this.busy=false;this.message=`${this.cues.length} ${this.t('subtitles')}`;}},error:error=>{this.stopProgress();this.busy=false;this.error=String(error?.error?.detail||error?.message||error);}});
  }
  importSrt(input:HTMLInputElement){const file=input.files?.[0];if(!file)return;this.importedSrtName=file.name;file.text().then(text=>{this.cues=this.parseSrt(text);this.current=this.cues.length?0:-1;this.dirty=!!this.cues.length;this.refreshNativeTrack();this.centerActiveCue(this.current);this.message=`${this.cues.length} ${this.t('subtitles')}`;});input.value='';}
  exportSrt(){this.cues=this.cues.map(c=>({...c,start:this.roundTime(c.start),end:this.roundTime(c.end)}));const content=this.cues.map((c,i)=>`${i+1}\n${this.srtTime(c.start)} --> ${this.srtTime(c.end)}\n${c.text}`).join('\n\n');this.download(content,this.exportFileName());this.refreshNativeTrack();this.dirty=false;this.message=this.t('saved');}
  reformat(){this.cues=this.format(this.cues);this.current=this.cues.length?Math.max(0,Math.min(this.current,this.cues.length-1)):-1;this.refreshNativeTrack();this.centerActiveCue(this.current);this.dirty=!!this.cues.length;}
  ...
  seek(index:number){const cue=this.cues[index];if(!cue||!this.player)return;this.current=index;this.centerActiveCue(index,true);this.player.nativeElement.currentTime=cue.start;void this.player.nativeElement.play();}
  sync(){const time=this.player?.nativeElement.currentTime||0;const active=this.cues.findIndex(c=>time>=c.start&&time<c.end);if(active>=0&&active!==this.current){this.current=active;this.centerActiveCue(active,true);}}
  ...
  private format(items:SubtitleSegment[]){const result:SubtitleSegment[]=[];for(const item of items){const words=String(item.text||'').trim().split(/\s+/).filter(Boolean);if(!words.length)continue;const blocks:string[]=[];let lines:string[]=[];let line='';for(const word of words){const next=line?`${line} ${word}`:word;if(next.length<=this.maxChars||!line)line=next;else{lines.push(line);line=word;if(lines.length===this.maxLines){blocks.push(lines.join(' '));lines=[];}}}if(line)lines.push(line);if(lines.length)blocks.push(lines.join(' '));const count=Math.max(blocks.length,Math.ceil(Math.max(.4,item.end-item.start)/this.maxDuration));const final=blocks.length===count?blocks:Array.from({length:count},(_,i)=>{const start=Math.floor(i*words.length/count);const end=Math.floor((i+1)*words.length/count);return words.slice(start,end).join(' ');}).filter(Boolean);final.forEach((text,i)=>result.push({start:this.roundTime(item.start+(item.end-item.start)*i/final.length),end:this.roundTime(item.start+(item.end-item.start)*(i+1)/final.length),text}));}return result;}
  ...
  private centerActiveCue(index:number,focusViewport=false){if(index<0)return;requestAnimationFrame(()=>requestAnimationFrame(()=>{const container=this.cueTable?.nativeElement,row=this.cueRows?.get(index)?.nativeElement;if(!container||!row)return;const box=container.getBoundingClientRect(),item=row.getBoundingClientRect();const top=container.scrollTop+(item.top-box.top)-(container.clientHeight-item.height)/2;const mobile=window.matchMedia('(max-width: 700px)').matches;container.scrollTo({top:Math.max(0,top),behavior:mobile?'auto':'smooth'});if(mobile&&focusViewport)requestAnimationFrame(()=>{const scroller=container.closest('main');if(!scroller)return;const activeBox=row.getBoundingClientRect(),viewport=scroller.getBoundingClientRect(),preview=scroller.querySelector<HTMLElement>('.preview');const reservedHeight=preview?.getBoundingClientRect().height||0;const readingHeight=Math.max(activeBox.height,scroller.clientHeight-reservedHeight);const desiredTop=reservedHeight+(readingHeight-activeBox.height)/2;const viewportTop=scroller.scrollTop+(activeBox.top-viewport.top)-desiredTop;scroller.scrollTo({top:Math.max(0,viewportTop),behavior:'auto'});});}));}
  private exportFileName(){const source=this.file?.name||this.importedSrtName||'subtitles';const stem=source.replace(/\.[^./\\]+$/,'').replace(/[<>:"/\\|?*\u0000-\u001f]/g,'-').trim()||'subtitles';return `${/^keltiawave-/i.test(stem)?'': 'keltiawave-'}${stem}.srt`;}
